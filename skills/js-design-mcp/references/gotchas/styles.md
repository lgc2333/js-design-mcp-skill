...
- CAUTION: Do not set `TextStyle.letterSpacing` through MCP for now, even though the typings say it is a `{ value, unit }` object. In the current JiShi plugin, writing that object can make the style editor show `[object Object]` in the letter-spacing field. Leave spacing untouched/default, or adjust it manually after visual inspection.
